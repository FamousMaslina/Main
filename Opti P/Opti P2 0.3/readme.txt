Opti P2 - an "OS" made in Python. (written for Version 0.3)
...
UpdateLog:
-Made bios.py recognize all mbs files. It works by detecting the mb variable, and creates another file, pointing 
to the identified filename, and it will continue to load just fine. So that means, you can make the most random mbs
and don't have to change the code, and you can name them however you want! (identifier.py does the job, and bios.py
and op2.py just reads idcpu.py and idmb.py)
-Fixed some identifier bugs
-Added new command to Opti P2: 'dvcman'
-Opti P2 now has commands loading times! (DON'T USE WEAK CPUS! I WARN YOU!)